...
// measure time
struct timeval t1, t2, t1a, t2a;
double elapsedTime0, elapsedTime1;
...
fix15 neighboring_boids[2] = {float2fix15(0), float2fix15(0)};
...
struct timespec target_framerate = {0, 33000};
...
void zeroAllAccum(int thread){
  xpos_avg[thread] = 0;
  ypos_avg[thread] = 0;
  xvel_avg[thread] = 0;
  yvel_avg[thread] = 0;
  neighboring_boids[thread] = 0;
  close_dx[thread] = 0;
  close_dy[thread] = 0; 
}

void boidSeparation(int i, int idx, int thread){
  ...
  if ( (abs(dx) < visualRange) && (abs(dy)< visualRange) ){
    ...
    if (squared_distance < protected_range_squared){
      ...
      xpos_avg[thread] += boid_array[idx].x;
      ypos_avg[thread] += boid_array[idx].y;
      xvel_avg[thread] += boid_array[idx].vx;
      yvel_avg[thread] += boid_array[idx].vy;
      neighboring_boids[thread] = neighboring_boids[thread] + float2fix15(1);
    }
  }
}
...
void boidCalculation(int idx, int thread){
  if(neighboring_boids[thread] > 0){
	  //printf("I fucked up\n");
    xpos_avg[thread] = divfix(xpos_avg[thread],neighboring_boids[thread]);
    ypos_avg[thread] = divfix(ypos_avg[thread],neighboring_boids[thread]);
    xvel_avg[thread] = divfix(xvel_avg[thread],neighboring_boids[thread]);
    yvel_avg[thread] = divfix(yvel_avg[thread],neighboring_boids[thread]);

    boid_array[idx].vx += multfix15((xvel_avg[thread] - boid_array[idx].vx),matchingFactor);
    boid_array[idx].vx += multfix15((xpos_avg[thread] - boid_array[idx].x ),centeringFactor);

    boid_array[idx].vy += multfix15((yvel_avg[thread] - boid_array[idx].vy),matchingFactor);
    boid_array[idx].vy += multfix15((ypos_avg[thread] - boid_array[idx].y ),centeringFactor);
  }

  boid_array[idx].vx += multfix15(close_dx[thread],avoidFactor);
  boid_array[idx].vy += multfix15(close_dy[thread],avoidFactor);
...
void boidUpdate(int idx, int thread){
  zeroAllAccum(thread);
  for(int i = 0; i < BOID_COUNT*2; i++){
    if(i != idx){
      boidSeparation(idx, i, thread);
    }
  }
  boidCalculation(idx, thread); 
}
...
void *animation_loop(void *arguments){
	...
	{	
      ...
      //printf("enter loop\n");
      for (int i = 0; i < BOID_COUNT*2; i=i+2) 
      {
        // erase boid
	  ...
	  elapsedTime0 = (t2.tv_sec - t1.tv_sec) * 1000000.0;      // sec to us
	  elapsedTime0 += (t2.tv_usec - t1.tv_usec) ;   // us 
		sprintf(time_string, "T = %6.0f uSec  ", elapsedTime0);
		VGA_text (10, 4, time_string);
	  struct timespec temp;
	  temp.tv_sec = 0;
	  temp.tv_nsec = (33000 - elapsedTime0) * 1000;
...
void *animation_loop_noprint(void *arguments){
	int index = *((int *) arguments);
	printf("%d\n", index);
	VGA_box(0, 0,  639, 479, 0x0000);
	while(1) 
	{	
	  ...
	  elapsedTime1 = (t2a.tv_sec - t1a.tv_sec) * 1000000.0;      // sec to us
	  elapsedTime1 += (t2a.tv_usec - t1a.tv_usec) ;   // us 
	  
	  struct timespec temp;
	  temp.tv_sec = 0;
	  temp.tv_nsec = (33000 - elapsedTime1) * 1000;
...
int main(void)
{
	...
	// so color = B+(G<<5)+(R<<11);
		...
		// Spawn a boid
    for(int i = 0; i < BOID_COUNT*2; i=i++){
	...
	int thread_args[2];
	int i;
	int result_code0, result_code1;
	for(i = 0; i < 2; i++){
		thread_args[i] = i;
	}
	result_code0 = pthread_create(&threads[0], NULL, animation_loop, &thread_args[0]);
	result_code1 = pthread_create(&threads[1], NULL, animation_loop_noprint, &thread_args[1]);
	assert(!result_code0 && "Failure in thread spawn");
